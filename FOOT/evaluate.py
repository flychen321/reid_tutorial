...
def evaluate(qf, ql, qc, gf, gl, gc, gn=None, q_index=0):
    query = qf.view(-1, 1)
    # print(query.shape)
    ...
    CMC_tmp = compute_mAP(index, qc, good_index, junk_index, gn, q_index)
    return CMC_tmp


def compute_mAP(index, qc, good_index, junk_index, gn, q_index):
    global dir_name
    ap = 0
    ...
    # print(query_name[q_index])
    # if os.path.exists('data/foot/retrieval_result'):
    #     shutil.rmtree('data/foot/retrieval_result')
    # os.mkdir('data/foot/retrieval_result')
    # for i in np.arange(200):
    #     name = str(i).zfill(2) + '_' + gn[index][i].strip()
    #     shutil.copy(os.path.join('data/foot/pytorch/gallery/new_wzj', gn[index][i].strip()),
    #                 os.path.join('data/foot/pytorch/gallery/new_wzj', name))
    # exit()
...
query_feature = torch.FloatTensor(result_query['query_f'])
query_name = result_query['query_name']
query_cam = result_query['query_cam'][0]
gallery_feature = torch.FloatTensor(result_gallery['gallery_f'])
gallery_name = result_gallery['gallery_name']
gallery_cam = result_gallery['gallery_cam'][0]
if is_foot:
    query_label = result_query['query_label']
    gallery_label = result_gallery['gallery_label']
else:
...
for i in range(len(query_label)):
    ...
    if i % 100 == 0 or i == len(query_label) - 1:
        ...
        i, CMC_tmp[0].numpy(), float(right_cnt - former_right_cnt) / (i - former_i + 1e-7), float(right_cnt) / (i + 1)))
