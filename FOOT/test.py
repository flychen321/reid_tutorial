...
parser.add_argument('--test_dir', default='gait', type=str, help='./test_data')
...
def get_id(img_path):
    ...
    for path, v in img_path:
        ...
        if 'foot' in opt.test_dir or 'gait' in opt.test_dir:
            label = os.path.dirname(path)[-5:]
        else:
            label = filename[0:4]
        if 'foot' in opt.test_dir or 'gait' in opt.test_dir:
            camera = 0
...
if 'query' in dataset_list:
    result_query = {'query_f': dataset_feature[1].numpy(), 'query_label': dataset_label[1], 'query_name': dataset_filename[1],
              'query_cam': dataset_cam[1]}
    scipy.io.savemat('pytorch_result_query.mat', result_query)

if 'gallery' in dataset_list:
    result_gallery = {'gallery_f': dataset_feature[0].numpy(), 'gallery_label': dataset_label[0],
